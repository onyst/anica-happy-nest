...
- **Suggested command**: Which command to use (prefer: the Commands table in SKILL.md)
...
1. **[P?] `npx impeccable command-name`**: Brief description (specific context from audit findings)
2. **[P?] `npx impeccable command-name`**: Brief description (specific context)

**Rules**: Only recommend commands from: the Commands table in SKILL.md. Map findings to the most appropriate command. End with `npx impeccable polish` as the final step if any fixes were recommended.

After presenting the summary, tell the user:

> You can ask me to run these one at a time, all at once, or in any order you prefer.
>
> Re-run `npx impeccable audit` after fixes to see your score improve.
